...
## Overview
Build the v6 topic-centered live-update editor as the primary production path while keeping the legacy `live_update_*` tables as rollback state. The latest critique does not show that the plan is targeting the wrong code or wrong root cause: the root problem is still the legacy editor's prompt/JSON/candidate architecture, and the requested fix is still a new topic editor. The remaining issue is narrower: the rejected-call audit model needs to match the locked sprint schema without inventing unapproved rejected action names.
...
Rejected-call scope is now explicit: the locked `rejected_*` actions are only `rejected_post_simple`, `rejected_post_sectioned`, and `rejected_watch`. Invalid `update_topic_source_messages` and `discard_topic` calls must not invent `rejected_update_*` or `rejected_discard` actions. They should return tool errors and write audit rows using their base actions (`update_sources` or `discard`) with `payload.outcome='tool_error'` and `payload.error_type=...`; idempotent replays return the prior result and are logged as idempotent/no-op only when needed for traceability, not as `rejected_*` rows.
...
9. Define `topic_transitions.action` to allow accepted/base actions (`post_simple`, `post_sectioned`, `watch`, `update_sources`, `discard`, `override`, `observation`) plus only the locked rejected actions (`rejected_post_simple`, `rejected_post_sectioned`, `rejected_watch`). Do not add `rejected_update_sources` or `rejected_discard` unless the sprint decisions are explicitly changed.
10. Keep `topic_id` nullable for rejected create attempts and non-null where available for base-action tool-error audits.
...
### Step 7: Implement dispatcher invariants and audit mapping (`src/features/summarising/topic_editor.py`)
**Scope:** Large
1. Reject `post_simple_topic` when source messages include `>= 2` distinct authors or `>= 3` source messages; write `topic_transitions.action='rejected_post_simple'` with `topic_id=NULL` and rejection details in `payload`.
2. Run canonical/alias/similarity collision scans for every `post_simple_topic`, `post_sectioned_topic`, and `watch_topic` write, returning a tool error unless matching `override_collisions` are provided. Collision rejections use `rejected_post_simple`, `rejected_post_sectioned`, or `rejected_watch` according to the attempted tool.
3. Enforce idempotency on `(run_id, tool_call_id)` before applying write side effects. Replays return the previously recorded tool result and do not create duplicate rejected rows.
...
5. For invalid `update_topic_source_messages` calls, return a tool error and write a `topic_transitions` row with `action='update_sources'`, `topic_id` when resolvable, `to_state=NULL`, and `payload.outcome='tool_error'`. Do not invent `rejected_update_sources`.
6. For invalid `discard_topic` calls, return a tool error and write a `topic_transitions` row with `action='discard'`, `topic_id` when resolvable, `to_state` unchanged or NULL, and `payload.outcome='tool_error'`. Do not invent `rejected_discard`.
7. For accepted actions, write normal transition rows using `post_simple`, `post_sectioned`, `watch`, `update_sources`, `discard`, `override`, or `observation` as appropriate.
8. Cap `record_observation` by prompt guidance to roughly 3/run and store observations for later 30-day retention cleanup.
...
2. Test all four dispatcher invariants, including the v6 collision override path and rejected transition logging for `post_simple`, `post_sectioned`, and `watch`.
3. Test invalid `update_topic_source_messages` and `discard_topic` calls are logged with base actions plus `payload.outcome='tool_error'`, not invented rejected action names.
4. Test idempotent replay of the same `(run_id, tool_call_id)` returns the prior result without duplicate transitions.
5. Test `topic_sources` duplicate message behavior.
6. Test `render_topic` for simple, sectioned, story-update, and partial publish failure handling.
7. Test active-run lease behavior: concurrent acquire is rejected, completed runs do not block future runs, and stale running rows can be expired before a new run starts.
...
2. Lock the transition action vocabulary in the migration and dispatcher before implementing tool handlers, especially the base-action audit mapping for invalid update/discard calls.
3. Finish migrations and backfill before runtime code depends on new tables.
4. Build `storage_handler.py` helpers and matching `db_handler.py` wrappers before the editor dispatcher.
5. Implement canonicalizer/collision logic before the LLM loop; these are deterministic and cheap to test.
6. Implement tool dispatch, lease lifecycle, and renderer before runner cutover.
7. Update admin/health surfaces before prod flip so operators do not lose status visibility.
8. Wire `SummarizerCog` last, preserving the injection seam used by tests.
9. Edit the runbook before publishing ON so it no longer contains contradictory gate instructions.
10. Run publishing-off replay before enabling prod publishing.
11. Mirror checkpoints before rollback or cutover so neither editor reprocesses the flip window.

## Validation Order
1. SQL lint/dry-run checks for schema files, including the exact `topic_transitions.action` vocabulary.
2. Focused unit tests for canonicalization, dispatcher invariants, rejected/base-action audit mapping, lease behavior, renderer, and storage fakes.
