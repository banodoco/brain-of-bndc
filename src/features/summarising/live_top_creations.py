...
class LiveTopCreations:
    ...
    async def run_once(self, trigger: str = "scheduled") -> Dict[str, Any]:
        ...
        try:
            ...
            # Advance the cursor only as far as the messages actually PROCESSED
            # this run. When more candidates were pending than the safety belt
            # allows, the overflow must remain eligible next run — anchoring on
            # the whole batch's newest message would skip it forever.
            checkpoint_messages = messages
            if len(pending) > LiveTopCreations.MAX_POSTS_SAFETY_BELT:
                processed_message_ids = {
                    candidate["source_message_id"]
                    for candidate in processed_candidates
                    if candidate.get("source_message_id") is not None
                }
                checkpoint_messages = [
                    message for message in messages
                    if message.get("message_id") in processed_message_ids
                ]
