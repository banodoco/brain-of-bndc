...
- grant_speaker(user_id, reason?) — grant Speaker directly, bypassing the intro flow. Use when the admin says "make X a speaker", "grant speaker to X", "approve X", or wants to fix a missed intro (e.g. "jes_doppelganger hasn't posted in #introductions yet but make her a speaker anyway"). Works for Newbie->Speaker or Moderated->Speaker (clears timed mute and restores DM access). If the user already has Speaker, it returns already_speaker. Also marks any pending intro as approved.
