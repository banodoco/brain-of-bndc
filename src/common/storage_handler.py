...
class StorageHandler:
    ...
    async def get_recently_approved_intros(self, hours: int = 24, guild_id: Optional[int] = None) -> List[Dict]:
        """Return pending_intros rows approved within the last ``hours`` hours.

        Mirrors ``DatabaseHandler.get_recently_approved_intros`` (which queries
        the same table through its storage handler) so the daily digest can
        answer "who was granted Speaker in the past 24 h" without unwrapping to
        the DatabaseHandler. Each row carries ``member_id`` and ``approved_at``.
        """
        if not self.supabase_client:
            return []
        try:
            ...
            query = (
                self.supabase_client.table('pending_intros')
                .select('*')
                .eq('status', 'approved')
    ...
    async def get_topic_sources(
        ...
        """Return the source-message rows for a topic (``topic_sources``).

        Sources are the Discord messages the topic was built from — the ground
        truth for any later fact-check of the published update. Returns raw
        rows (message_id, guild_id, added_in_run_id, created_at, ...) newest
        first, or [] when the topic has no recorded sources.
        """
        if not self.supabase_client or not topic_id:
            return []
        safe_limit = max(1, min(int(limit or 50), 100))
        try:
            result = await asyncio.to_thread(
                self.supabase_client.table('topic_sources')
                .select('message_id, guild_id, added_in_run_id, created_at')
            ...
            logger.error(
                f"Error fetching topic_sources for topic_id={topic_id}: {e}",
                exc_info=True,
            )
            return []

    async def get_topic_ground_truth(
        self,
        topic_id: str,
        ...
        Chains ``topic_sources`` → ``discord_messages`` (the archive) and
        returns a compact per-message shape — message_id, channel_id,
        author_id, content (truncated to 1500 chars), created_at,
        reference_id, reaction_count — ordered by source recency. Empty when
        the topic has no sources or the archive has none of them.

        This is what the community-feedback agent fact-checks the posted
        update against before editing or deleting it.
        """
        source_rows = await self.get_topic_sources(topic_id, environment=environment, limit=limit)
        ids = [
        ...
        if not ids:
            return []
        rows = self.get_topic_editor_source_messages(
            ids,
            guild_id=guild_id,
    ...
    async def get_recent_topic_editor_drafts(
        ...
        try:
            ...
            query = (
                ...
                .select('*')
                .eq('environment', environment)
                .order('updated_at', desc=not ascending)
                .limit(safe_limit)
            )
            if guild_id is not None:
                query = query.eq('guild_id', guild_id)
